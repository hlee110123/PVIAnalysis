WITH cohort_patients AS (
    ...
    SELECT
        ...
        cohort_start_date,
        CASE
            WHEN cohort_definition_id = @cohort_definition_id_1 THEN 'Early PVI'
            WHEN cohort_definition_id = @cohort_definition_id_2 THEN 'Non-Early PVI'
        END as cohort_group,
        CASE
            WHEN cohort_definition_id = @cohort_definition_id_1 THEN DATE_ADD(cohort_start_date, 183)
            WHEN cohort_definition_id = @cohort_definition_id_2 THEN DATE_ADD(cohort_start_date, 183)
        END as outcome_start_date
    FROM @cohort_database_schema.@cohort_table
    WHERE cohort_definition_id IN (@cohort_definition_id_1, @cohort_definition_id_2)
),
...
clti_events AS (
    ...
    LEFT JOIN @cdm_database_schema.condition_occurrence co
...
SELECT
    ...
    DATEDIFF(day, cp.cohort_start_date, COALESCE(d.death_date, '2022-12-31')) as follow_up_days,
...
FROM cohort_patients cp
LEFT JOIN @cdm_database_schema.death d ON cp.person_id = d.person_id
